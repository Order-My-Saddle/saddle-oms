...
export async function fetchUsers({
  ...
  console.log('🔍 fetchUsers: Using efficient NestJS backend API...');
  ...
  if (!response.ok) {
    ...
    console.error('🔍 fetchUsers: Backend error:', response.status, errorText);
    throw new Error(`Failed to fetch users: ${response.status} ${response.statusText}`);
  }

  const result = await response.json();
  console.log('🔍 fetchUsers: Backend response:', {
...
export async function createUser(userData: CreateUserData): Promise<User> {
  ...
  console.log('Attempting user creation with BreezeJS SaveBundle:', JSON.stringify(saveBundle, null, 2));
...
export async function updateUser(id: string, userData: UpdateUserData): Promise<User> {
  ...
  console.log('Attempting user update with BreezeJS SaveBundle:', JSON.stringify(saveBundle, null, 2));
  ...
  // Helper function to map backend user to frontend User type
  const mapBackendUserToFrontend = (backendUser: any): User => {
    console.log('🔍 Mapping backend user (update):', backendUser);
    ...
    console.log('🔍 Name mapping (update):', {
      originalName: backendUser.name,
      nameParts,
      firstName,
      lastName
...
export async function fetchUserCount(): Promise<number> {
  try {
    ...
    if (!response.ok) {
      console.error('📊 Error fetching user count:', response.status, response.statusText);
      return 0;
    }
