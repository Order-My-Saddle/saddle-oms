...
describe("Auth Module", () => {
  ...
  describe("Registration", () => {
    it("should fail with existing username/email: /api/v1/auth/email/register (POST)", () => {
      return request(app)
        .post("/api/v1/auth/email/register")
        .send({
          email: TESTER_USERNAME,
    ...
    it("should successfully for user with confirmed email: /api/v1/auth/email/login (POST)", () => {
      return request(app)
        ...
        .expect(({ body }) => {
          ...
          expect(body.user.enabled).toBeDefined();
  ...
  describe("Logged in user", () => {
    ...
    it("should update profile email successfully: /api/v1/auth/me (PATCH)", async () => {
      // User name for profile update test
      void `Tester ${Date.now()}`;
      const newUserEmail = `user.${Date.now()}@example.com`;
      const newUserPassword = `secret`;
